...
class login(Frame):
    ...
    def createPage(self):
        ...
        for i in range(0,3):
            Label_fillup.append(Label(self, bg = "AntiqueWhite"))
            Label_fillup[i].pack(anchor = CENTER)

        Label_username = Label(self, text = "please enter your username",bg = "AntiqueWhite", font = font_label_fg)
    ...
    def loginCallBack(self):
        ...
        if result:
            self.destroy()
            Frame_home = mainFrame(self.cvs, self.username.get(), self.root)
            self.cvs.create_window(500, 215, width = 500, height = 430, window = Frame_home)
        else:
