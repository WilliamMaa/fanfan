...
class mainFrame(Frame):
    ...
    def createFrame(self):
        ...
        self.Label_profile_photo = Label(Frame_userInformation, image=render)
        self.Label_profile_photo.image = render
        self.Label_profile_photo.grid(row = 0, column = 2)
        ...
        Button_moreinfo = Button(Frame_appointment, image = render_plus, command = self.moreInfoCallback)
    ...
    def makeAppointmentCallBack(self):
        ...
        self.cvs.create_window(500, 215, width=550, height=430, window=self.Frame_appointment)
    ...
    def moreInfoCallback(self):
        self.destroy()
        appointment_info = appointmentInfoMain(self.cvs, self.username, self.root)

        self.cvs.create_window(500, 215, width = 500, height = 430, window = appointment_info)
    ...
    def on_closing_setting(self):
        ...
        for i in file:
            if i.split(";")[0] == self.username:
                image_address = i.split(";")[1].replace("\n", "")
                break

        file.truncate()
        file.close()

        load = Image.open(image_address)
        load = self.resize(100, 100, load)
        render = ImageTk.PhotoImage(load)
...
class editPreference(Frame):
    ...
    def acceptCallBack(self):
        ...
        if tasteUpadate and priceUpdate:
            ...
            if not callBack:
                self.root.destory()
                editInfoMain(self.username, self.b1, self.b2, self.root)
        else:
    ...
    def cancelCallback(self):
        self.destroy()
        editInfoMain(self.username, self.b1, self.b2, master = self.root)
...
class editPassword(Frame):
    ...
    def acceptCallback(self):
        if not self.password == self.password_ori_var.get():
            ...
            userInformation.updatePassword(self.username, self.password_new_var.get())
            messagebox.showinfo("Update success!", "Your password has been changed!")
            self.destroy()
            editInfoMain(self.username, self.b1, self.b2, master = self.root)
...
class editInfoMain(Frame):
    def __init__(self, username, b1 = None, b2 = None, master = None):
        Frame.__init__(self, master, bg = 'AntiqueWhite')
        self.username = username
        self.root = master
        ...
        self.imageAddress_new = StringVar()
    ...
    def createFrame(self):
        ...
        Label_fillup = []
        for i in range(0,1):
            Label_fillup.append(Frame(self.Frame_main, bg = "AntiqueWhite"))
            Label_fillup[i].pack(anchor = NW, ipady = 5)

        Frame_photo = Frame(self.Frame_main, bg = "AntiqueWhite")
        Frame_photo.pack(anchor = NW, ipadx = 31)
        Frame_username = Frame(self.Frame_main, bg = "AntiqueWhite")
        Frame_username.pack(anchor = NW, ipadx = 41)
        Frame_password = Frame(self.Frame_main, bg = "AntiqueWhite")
        Frame_password.pack(anchor = NW, ipadx = 71)
        Frame_user_info = Frame(self.Frame_main, bg = "AntiqueWhite")
        Frame_user_info.pack(anchor = NW, ipadx = 42)
        Frame_restaurant_pref = Frame(self.Frame_main, bg = "AntiqueWhite")
        Frame_restaurant_pref.pack(anchor = NW, ipadx = 12)
        Frame_accept_cancel = Frame(self.Frame_main, bg = "AntiqueWhite")
        Frame_accept_cancel.pack(anchor = NW, ipadx = 60)
        ...
        Label_profile_photo.pack(anchor = N, side = LEFT, ipady = 40, ipadx = 5)
        ...
        self.Button_photo = Button(Frame_photo, image = render_profile_photo, bd = 0, command = self.editProfilePhoto)
        self.Button_photo.image = render_profile_photo
        self.Button_photo.pack(anchor = N, side = RIGHT)
        ...
        Label_username.pack(anchor = N, side = LEFT, ipadx = 5)
        ...
        Label_password.pack(anchor = N, side = LEFT, ipadx = 5)
        ...
        Label_user_info.pack(anchor = N, side = LEFT, ipadx = 5)
        ...
        Label_restaurant_pref.pack(anchor = N, side = LEFT, ipadx = 5)
        ...
        Label_fillup.append(Label(Frame_restaurant_pref, bg = "AntiqueWhite"))
        Label_fillup[-1].pack(ipady = 15)

        for i in range(0,2):
            Label_fillup.append(Label(Frame_accept_cancel, bg = "AntiqueWhite", width = 1))
            Label_fillup[-1].grid(row = 0, column = i, ipadx = 2)

        Label_fillup.append(Label(Frame_accept_cancel, bg = "AntiqueWhite", width = 4))
        Label_fillup[-1].grid(row = 0, column = 4)

        Button_accept = Button(Frame_accept_cancel, text = "Accept", bg = "LightGreen", bd = 3, font = font_button, command = self.acceptCallback)
        Button_accept.grid(row = 0, column = 5, ipadx = 15, ipady = 6)
        Button_cancel = Button(Frame_accept_cancel, text = "Cancel", bg = "WhiteSmoke", bd = 3, font = font_button, command = self.cancelCallback)
        Button_cancel.grid(row = 0, column = 3, ipadx = 15, ipady = 6)

    def editPersonalInfoCallback(self):
        self.Frame_main.destroy()
        personal_info = editUserInfo(self.username, self.b1, self.b2, self.root)
        personal_info.pack()

    def editPreferenceCallbac(self):
        self.Frame_main.destroy()
        preference_info = editPreference(self.username, self.b1, self.b2, self.root)
        preference_info.pack()

    def editPassword(self):
        self.Frame_main.destroy()
        password_info = editPassword(self.username, self.b1, self.b2, self.root)
        password_info.pack()

    def editProfilePhoto(self):
        select_file = filedialog.askopenfilename(title = "Select new profile photo")
        if select_file != "":
            self.imageAddress_new.set(select_file)
            load = Image.open(select_file)

            load = self.resize(100, 100, load)
            render = ImageTk.PhotoImage(load)

            self.Button_photo['image'] = render
            self.Button_photo.image = render

            userInformation.updatePhotoInfo(self.username, self.imageAddress_new.get())
    ...
    def acceptCallback(self):
        if not self.username == self.usernameVar.get() and self.usernameVar.get() != "":
            ...
            file.close()
            messagebox.showinfo("Update success!", "All change had been saved!")
        else:
...
class appointmentInfoMain(Frame):
    def __init__(self, cvs, username, master = None):
        ...
        self.appointment_info = appointmentInfo.getAppointment(self.username)

        self.createFrame()

    def createFrame(self):
        font_main = font.Font(family = "Arial", size = 18, weight = font.BOLD)
        font_sub_title = font.Font(family = "Arial", size = 13)
        font_label = font.Font(family = "Arial", size = 10)
        ...
        Label_main_info = Label(self, text = "All Appointments", bg = "AntiqueWhite", font = font_main, fg = "NavyBlue")
        Label_main_info.pack()

        # everything except the main title is on this frame
        Frame_main = Frame(self, bg = "AntiqueWhite")
        Frame_main.pack()

        # everything about pending appointment is on this frame
        Frame_pending = Frame(Frame_main, bg = "AntiqueWhite")
        Frame_pending.pack(ipadx = 150)

        # the pending title and expand button is on this frame
        Frame_pending_title = Frame(Frame_pending, bg = "AntiqueWhite")
        Frame_pending_title.pack(ipadx = 150)

        # adds a blank space between main_title and sub_title
        Label_fillup.append(Label(Frame_pending, bg = "AntiqueWhite"))
        Label_fillup[-1].pack(before = Frame_pending_title)

        Label_pending = Label(Frame_pending_title, text = "Pending", font = font_sub_title, bg = "AntiqueWhite")
        Label_pending.pack(anchor = N, side = LEFT)

        load = Image.open('./plus.jpg')
        load = self.resize(20,20,load)
        render_plus = ImageTk.PhotoImage(load)

        Button_more_info_pending = Button(Frame_pending_title, image = render_plus, bg = "AntiqueWhite")
        Button_more_info_pending.image = render_plus
        Button_more_info_pending.pack(anchor = N, side = LEFT)

        # information about pending appointment is on this frame
        Frame_pending_info = Frame(Frame_pending, bg = "AntiqueWhite")
        Frame_pending_info.pack(ipadx = 10)

        # set the pending appointments available on Frame_pending_info frame
        appoitnments_pending = []
        count = 0
        for i in self.appointment_info:
            if i[3] == "Pending" and count < 3:
                dateInfo = appointmentInfo.listToDateInfo(i[1])
                appoitnments_pending.append(Label(Frame_pending_info, text = "with " + i[0] + " at " + i[2] + " on " + dateInfo, bg = "AntiqueWhite", font = font_label))
                appoitnments_pending[-1].pack(anchor = NW, ipady = 5)
                count += 1
        # fill up the blank space so that it totally takes three position
        while count < 3:
            Label_fillup.append(Label(Frame_pending_info, bg = "AntiqueWhite"))
            Label_fillup[-1].pack(anchor = NW, ipady = 5)
            count += 1

        Frame_goingto = Frame(Frame_main, bg = "AntiqueWhite")
        Frame_goingto_info = Frame(Frame_goingto, bg = "AntiqueWhite")
        appointments_goingto = []
        count = 0
        for i in self.appointment_info:
            if i[3] == "goingto" and count < 3:
                dateInfo = appointmentInfo.listToDateInfo(i[1])
                appointments_goingto.append(Label(Frame_pending_info, text = "with " + i[0] + " at " + i[2] + " on " + dateInfo, bg = "AntiqueWhite", font = font_label))
