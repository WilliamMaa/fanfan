...
def writeAppointment(appointment):
    ...
    if file.read() == "":
        toWrite = listToString(appointment) + ";Pending"
    else:
        toWrite = "\n" + listToString(appointment) + ";Pending"
...
def getAppointment(username):
    ...
    for i in appointments:
        if i[0] == username:
            result.append([i[1], i[2], i[3], i[4]])
        elif i[1] == username:
            result.append([i[0], i[2], i[3], i[4]])
...
def changeToGoingto(appointment):
    file = open('./Info/appointmentInfo.txt', 'r')
    toWrite = ""

    for i in file:
        temp = i.split(";")
        if utility.isSame(appointment, temp):
            end = utility.findSemiColon(i, 4) + 1
            toWrite += i[:end] + "GoingTo"
        else:
            toWrite += i
...
def changeToFinished(appointment):
    file = open('./Info/appointmentInfo.txt', 'r')
    toWrite = ""
