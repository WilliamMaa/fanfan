...
def updatePassword(username, password_new):
    file = open('./Info/userInfo.txt', 'r')
    ...
    for i in file:
        if i.split(";")[0] == username:
            ...
            toWrite += i[:start] + password_new + i[end:]
        else:
            toWrite += i
...
def updatePhotoInfo(username, profilePhoto_new):
    file = open('./Info/profilePhotoInfo.txt', 'r')
    ...
    for i in file:
        if i.split(";")[0] == username:
            ...
            toWrite += i[:start] + profilePhoto_new + "\n"
